...
class DBClient:
    ...
    def reset_climate_automation(self):
        self.tesla_database['tesla_climate_status'].update_one({"_id": "enum"}, {"$set": {"climate_state": 'at_user_well'}})
        self.tesla_database['tesla_climate_status'].update_one({"_id": "enum"}, {"$set": {"climate_turned_on_before": 'False'}})
